...
export default function ProductDetail({product}: {product:Product}) {
 ...
 async function addToCart() {
    const token = session?.user?.token; // 1. اسحبي التوكن هنا
    ...
    setIsLoading(true);
    try {
      // 2. ابعتي التوكن كـ Parameter تاني للدالة
      const response = await apiServices.addProductToCart(product.id, token);
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600 dark:text-gray-400">
                    {product.ratingAverage} ({product.ratingQuantity} reviews)
                  </span>
                </div>
              </div>
              <button 
            ...
            <div className="mt-8">
              <div className="flex items-center space-x-4">
                ...
                <button  onClick={addToCart}
                 disabled={isloading}
                  type="button"
                  className="flex-1 disabled:bg-indigo-400  bg-indigo-600 text-white py-3 px-6 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 flex items-center justify-center gap-2"
                >
